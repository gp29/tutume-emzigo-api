...
const create = async(requestParam) => {
    return new Promise(async(resolve, reject) => {
        try {
            let exists = await query.selectWithAndOne(dbConstants.dbSchema.installments, {product_id: requestParam.product_id, rider_id: requestParam.rider_id}, { _id: 0, product:1, name:1, installments:1}, { created_at: -1 });
            if(exists){
                let is_unpaid = false
                _.each(exists.installments, (itm) => {
