...
const payRiderInstallment = async(requestParam) => {
    return new Promise(async(resolve, reject) => {
        try {
            ...
            let inst = await query.selectWithAndOne(dbConstants.dbSchema.installments, {rider_id:requestParam.rider_id, product_id: requestParam.product_id, all_paid:'no'}, { _id:0, installment_id:1, product_id:1, installments:1} );
            ...
            if(val.length > 0){
                ...
                await query.updateSingle(dbConstants.dbSchema.installments, { installments, all_paid }, {installment_id: inst.installment_id});
